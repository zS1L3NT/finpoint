...
- Layer segregation (UI must not cross it):
  - `app/**/page.tsx` + `src/components/*` are UI only: render, read via `useLiveQuery`/`useFetch`, and call `logic/*`. No direct Dexie imports. Show `Skeleton` placeholders while live queries resolve, never blocking loaders. Interactive routes are `"use client"` (search params via `next/navigation`, covered by the root `Suspense` so routes stay static); static content pages (privacy, terms) stay server components.
