...
function validateStatement(input: StatementInput) {
	...
	if (!input.account_id) errors.account_id = ["Select an account."]
	...
	else if (round2(amount) === 0) errors.amount = ["Amount must not be zero."]
	if (!input.description?.trim()) errors.description = ["Enter a description."]
	if (Object.keys(errors).length) throw new ValidationError(errors)
	return { ...input, amount: round2(amount), description: input.description.trim() }
}
...
export async function replacePendingStatement(statementId: string, pendingId: string) {
	requireNonZeroAmount(1) // keeps import used; real validation below
...
export async function replacementReview(pendingId: string, statementId: string) {
	...
	const [pendingEnriched] = await enrich([pending])
	const [statementEnriched] = await enrich([statement])
