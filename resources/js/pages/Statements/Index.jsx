            ...
            <div className="d-flex mb-4">
                <Form method="post" action={route("records.store")}>
                    {selectedIds.map(selectedId => (
                        <input key={selectedId} type="hidden" name="statement_ids[]" value={selectedId} />
                    ))}
                    <button className="btn btn-primary" disabled={!selectedIds.length}>{selectedIds.length > 1 ? "Merge into" : "Create"} Record</button>
                </Form>
            </div>
            ...
            <table className="table table-hover">
                ...
                <tbody>
                    {statements.data.map(statement => (
                        <tr key={statement.id} className={`${selectedIds.includes(statement.id) ? "table-active" : ""}`} style={{ cursor: "pointer" }} onClick={e => {
                            e.preventDefault()
                            setSelected(statement.id, selectedIds.includes(statement.id))
                        }}>
                            <td>
                                <input type="checkbox" checked={selectedIds.includes(statement.id)} style={{ pointerEvents: "none", userSelect: "none" }} onChange={e => e.preventDefault()} />
                            </td>
