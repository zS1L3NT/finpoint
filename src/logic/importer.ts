...
async function readFile(file: File): Promise<Row[]> {
	...
	if (name.endsWith(".xlsx") || name.endsWith(".xls")) {
		...
		if (!sheet) throw new ValidationError({ files: ["Invalid CSV Format: empty workbook."] })
...
function parseDayMonthYear(value: string | null | undefined): string {
...
async function upsertIndexed(rows: Parsed[] & { index?: number }[], withIndex: boolean) {
	...
	for (const [, group] of unmatched) {
		...
		if (existing) {
			...
			const matchIdx = matchIndex === -1 ? 0 : matchIndex
			const match = group[matchIdx]
			if (!match) continue
			...
			reindexed++
			remaining = group.filter((_, i) => i !== matchIdx)
		}
